...
interface HeaderProps {
  ...
  viewMode: 'warmup' | 'trees' | 'stretches';
  isTimerOpen: boolean;
  onSelectTree: (treeId: string | null) => void;
  onViewModeChange: (mode: 'warmup' | 'trees' | 'stretches') => void;
  onToggleTimer: () => void;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-3.5 pb-2.5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <button
            ...
            className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-[#141414] hover:bg-[#1f1f1f] text-zinc-300 hover:text-[#D1FF00] border border-[#222222] hover:border-[#D1FF00]/40 transition cursor-pointer flex items-center gap-1.5 text-xs font-mono font-bold shadow-sm active:scale-95"
            title="Launch Guided Superset Workout Session"
          >
            <Dumbbell className="w-4 h-4 text-[#D1FF00]" />
          ...
          <button
            ...
            className={`p-2 sm:px-3 sm:py-1.5 rounded-xl border text-xs font-bold font-mono transition cursor-pointer flex items-center gap-1.5 shadow-sm ${
              isTimerOpen
                ? 'bg-[#D1FF00] text-black border-[#D1FF00] shadow-sm shadow-[#D1FF00]/20'
                : 'bg-[#141414] hover:bg-[#1f1f1f] text-zinc-300 hover:text-[#D1FF00] border-[#222222] hover:border-[#D1FF00]/40'
            }`}
            title="Toggle Rest Timer & Stopwatch"
          >
            <Timer className={`w-4 h-4 ${isTimerOpen ? 'text-black' : 'text-[#D1FF00]'}`} />
            <span className="hidden sm:inline">Chrono</span>
          </button>
        </div>
      </div>

      {/* Sub-bar directly under heading: Warmup, Trees, Stretches */}
